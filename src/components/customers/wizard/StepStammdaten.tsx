    ...
    <div className="space-y-6 mt-4">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2 col-span-3"><Label htmlFor="strasse">Straße und Hausnummer{isKunde ? ' *' : ''}</Label><Input id="strasse" value={customerData.strasse} onChange={(e) => setCustomerData((p: any) => ({ ...p, strasse: e.target.value }))} placeholder="Straße und Hausnummer" required={isKunde} /></div>
          <div className="space-y-2"><Label htmlFor="plz">PLZ{isKunde ? ' *' : ''}</Label><Input id="plz" value={customerData.plz} onChange={(e) => { const val = e.target.value.replace(/\D/g, '').slice(0, 5); setCustomerData((p: any) => ({ ...p, plz: val })); }} placeholder="30159" maxLength={5} inputMode="numeric" required={isKunde} /></div>
